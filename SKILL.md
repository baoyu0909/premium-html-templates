...
1. 如果用户只给了大概主题、素材或内容，不要先反问参数；直接自动判断最合适的输出模式、风格原型和色彩倾向，并按最优方案输出。
2. 先判断输出模式：`deck` 或 `single-page`。
3. 再判断用户要对齐哪一种风格原型。
4. 读取对应 mode reference 和 variant reference，不要一次性加载全部细节。
5. 输出时优先复用该组合的：
   - 页面宽高关系
   - 背景构成
   - 标题层级
...
- 如果用户只说“生成一个 HTML 汇报页 / HTML 页面”，默认根据内容自动选型，不要求用户提供英文参数。
...
- “数据复盘 / 经营分析 / 增长建议 / 执行计划” 优先 `light-report`

### 2.1 选色彩倾向

如果用户没有明确指定色彩倾向，自动按内容选择：

- “增长 / 重点动作 / 关键结论 / 决策 / 正式汇报” 优先红色主强调。
- “数据 / 系统 / 分析 / 科技 / 稳定” 优先蓝色主强调。
- “资源 / 渠道 / 卡片 / 多模块展示” 优先混合强调色。
- `light-report` 默认允许红色作为主强调色，不要默认做成全蓝。
